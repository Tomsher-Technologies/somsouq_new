<div class="tab">
    <div class="col-md-8 m-auto">
        <div class="post-ad-place" >
            <h4>Tell Us About Your Heavy Equipment</h4>
            <input type="hidden" name="post_detail_id" value="{{ $postDetail->id ?? "" }}">

            <div class="row g-3">
                <div class="col-md-6">
                    <select class="form-select" aria-label="Default select example" name="heavy_equipment_type_id">
                        <option value="">Select Heavy Equipment Type</option>
                        @foreach($heavyEquipmentTypes as $key => $type)
                            @if($postDetail)
                                <option value="{{ $key }}" @selected($key == $postDetail->heavy_equipment_type_id ?? "")>{{ $type }}</option>
                            @else
                ...
                <div class="col-md-6">
                    <input type="number" class="form-control" id="formGroupExampleInput" name="km" placeholder="Km" {{ $postDetail->km ?? "" }}>
                </div>
                ...
                <div class="col-md-6">
                    <input type="number" class="form-control" id="formGroupExampleInput" name="carriage_capacity" placeholder="Capacity/ Weight" value="{{ $postDetail->carriage_capacity ?? "" }}">
                </div>
                ...
                <div class="col-md-6">
                    <select class="form-select" aria-label="Default select example" name="usage_condition">
                        ...
                        <option value="damaged" {{ ($postDetail->usage_condition ?? "" == "damaged") ? "selected" : "" }}>Damaged</option>
                    </select>
                </div>
