...
<div class="tab-content" id="myTabContent">
    <div class="tab-pane fade {{ (getLocaleLang() == 'so') ? "show active" : "" }}" id="somali" role="tabpanel" aria-labelledby="somali-tab">
        <div class="row g-3">
            ...
            <div class="col-md-6">
                <select class="form-select {{ isEvent('so') }}" aria-label="Default select example" id="connectivity_so" name="connectivity_so" {{ isDisable('so') }}>
                    <option value="">{{ __('post.connectivity') }}</option>
                    @foreach($connectivities as $key => $connectivity)
                        @if($postDetail)
                            <option value="{{ $key }}" @selected($key == $postDetail->connectivity ?? "")>{{ $connectivity }}</option>
                        @else
                            <option value="{{ $key }}">{{ $connectivity }}</option>
                        @endif
                    @endforeach
                </select>
            </div>
    ...
    <div class="tab-pane fade {{ (getLocaleLang() == 'en') ? "show active" : "" }}" id="english" role="tabpanel" aria-labelledby="english-tab">
        <div class="row g-3">
            ...
            <div class="col-md-6">
                <select class="form-select {{ isEvent('en') }}" aria-label="Default select example" id="connectivity_en" name="connectivity_en" {{ isDisable('en') }}>
                    <option value="">{{ __('post.connectivity') }}</option>
                    @foreach($connectivities as $key => $connectivity)
                        @if($postDetail)
                            <option value="{{ $key }}" @selected($key == $postDetail->connectivity ?? "")>{{ $connectivity }}</option>
    ...
    <div class="tab-pane fade {{ (getLocaleLang() == 'ar') ? "show active" : "" }}" id="arabic" role="tabpanel" aria-labelledby="arabic-tab">
        <div class="row g-3">
            ...
            <div class="col-md-6">
                <select class="form-select {{ isEvent('ar') }}" aria-label="Default select example" id="connectivity_ar" name="connectivity_ar" {{ isDisable('ar') }}>
                    <option value="">{{ __('post.connectivity') }}</option>
                    @foreach($connectivities as $key => $connectivity)
                        @if($postDetail)
                            <option value="{{ $key }}" @selected($key == $postDetail->connectivity ?? "")>{{ $connectivity }}</option>
